...
const commentController = {
    ...
    create: async (req, res) => {
        try {
          ...
          const commentValues = [index, previousHash, timestamp, JSON.stringify(data), ipAddress, hash, "active"];
          ...
          const historyValues = [index, previousHash, timestamp, JSON.stringify(data), ipAddress, hash, 1];
      
          const historyResult = await postgre.query(historySql, historyValues);
      
          res.status(201).json({ message: 'เพิ่ม comment และ comment-history สำเร็จ', data: { comment: commentResult.rows[0], history: historyResult.rows[0] } });
        } catch (error) {
          res.status(500).json({ error: 'ไม่สามารถเพิ่ม comment และ comment-history ได้' });
        }
      },      
    deleteById: async (req, res) => {
        try {
          ...
          const deleteSql = 'UPDATE comment SET isDeleted = $1, deleteTime = NOW() WHERE id = $2';
